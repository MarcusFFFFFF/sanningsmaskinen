# -*- coding: utf-8 -*-
"""
SANNINGSMASKINEN v8.17 — STREAMLIT UI
Designmål:
- Matcha referensdesignens visuella hierarki så nära som möjligt i Streamlit
- Behåll befintlig engine/normalizer/pipeline intakt
- Visa tydlig, kompakt och klickbar analys
"""
...
:root{
  ...
  --ink:#e2ddd6; --ink2:#a09a92; --ink3:#5a5650; --ink4:#2e2c33;
...
.exec-explanation{font-family:var(--sans);font-size:.8rem;color:var(--ink3);line-height:1.65;padding:.6rem 1.1rem;border-left:2px solid var(--border2);background:var(--bg2);}
...
.exec-falsif-txt{font-family:var(--sans);font-size:.78rem;color:#7070a0;line-height:1.55;}
...
.hyp-card-tes{font-family:var(--sans);font-size:.75rem;color:var(--ink3);line-height:1.55;margin-top:.5rem;padding-top:.5rem;border-top:1px solid var(--border);}
...
.hyp-tes{font-family:var(--sans);font-size:.8rem;color:#7ab89a;line-height:1.6;padding:.4rem .7rem;border-left:2px solid var(--grn-dim);background:var(--grn-bg)}
.hyp-ev-list{list-style:none;padding:0;margin:0}
.hyp-ev-list li{font-family:var(--sans);font-size:.77rem;color:var(--ink3);line-height:1.5;padding:.28rem .7rem;border-left:2px solid var(--grn-dim);background:var(--grn-bg);margin-bottom:.15rem;display:flex;gap:.4rem;align-items:baseline}
.hyp-ev-list li::before{content:"•";color:var(--grn);flex-shrink:0}
.hyp-mo{font-family:var(--sans);font-size:.77rem;color:#9a7060;line-height:1.5;padding:.28rem .7rem;border-left:2px solid var(--red-dim);background:var(--red-bg);margin-bottom:.15rem}
.hyp-fl{font-family:var(--sans);font-size:.77rem;color:#7070a0;line-height:1.5;padding:.3rem .7rem;border-left:2px solid var(--blu-dim);background:var(--blu-bg)}
...
.primary-intro-t{font-family:var(--sans);font-size:.82rem;color:var(--ink3);line-height:1.7}
.analysis-text{font-family:var(--sans);font-size:.79rem;line-height:1.8;color:var(--ink3)}
...
.conflict-line{font-size:.77rem;color:var(--ink3);padding:.2rem 0;border-bottom:1px solid var(--border);line-height:1.52}
...
STYRKA_COLOR = {"HÖG": "#3d9970", "MEDEL-HÖG": "#6daa50", "MEDEL": "#4a7fc0", "LÅG": "#c0442a"}
REALITY_PILL = {
    "VERIFIED": ("grn", "✓ VERIFIED"), "ONGOING": ("blu", "◉ ONGOING"), "PARTIAL": ("amb", "◑ PARTIAL"),
    "HYPOTHETICAL": ("dim", "◌ HYPOTHETICAL"), "ANALYTICAL": ("blu", "◎ ANALYTICAL"), "UNVERIFIED": ("red", "✗ UNVERIFIED"), "ERROR": ("red", "✗ ERROR"),
}
STATUS_PILL = {"KLAR": ("grn", "✓ KLAR"), "REVIDERAD": ("amb", "↻ REVIDERAD"), "DEGRADERAD": ("red", "⚠ DEGRADERAD"), "RUNNING": ("blu", "◉ RUNNING")}
...
def _hyp_dashboard_html(ranked):
    ...
    for i, h in enumerate(ranked[:4]):
        ...
        styrka = (h.get("styrka") or "MEDEL").upper(); tes = h.get("tes", "")
        bevis = h.get("bevis", []) or []; motarg = h.get("motarg", []) or []
        pct = int(h.get("conf_pct", int(h.get("conf", 0.5) * 100)))
        color = STYRKA_COLOR.get(styrka, "#4a7fc0"); cls = card_classes[i] if i < len(card_classes) else ""
        tes_short = (tes[:130] + ("…" if len(tes) > 130 else "")) if tes else ""
        cards.append(f"""
...
def _nyckelord_html(ranked):
    ...
    for h in ranked[:3]:
        ...
        if term:
            ...
            tags.append(f'<a class="nyckelord-tag" href="{url}" target="_blank" rel="noopener noreferrer">{_safe(term)}</a>')
    if not tags:
        return ""
...
def _parse_rc_structured(txt: str):
    ...
    for line in (txt or "").split("\n"):
        ...
        elif any(x in u for x in ["DISPUTED", "OMTVISTAD", "DELVIS", "PARTIAL", "OKLART", "◑"]): st = "DISPUTED"
...
def _rc_table_html(items):
    ...
    for it in items:
        ...
        elif "DISPUTED" in status or "OMTVISTAD" in status or "PARTIAL" in status: sc, si = "rc-st-d", "◑ DISPUTED"
        elif "UNVERIFIED" in status or "EJ BEKRÄFTAD" in status: sc, si = "rc-st-u", "✗ UNVERIFIED"
        else: sc, si = "rc-st-n", "◎ —"
...
def _verdict_from_red_team(red_text: str):
    ...
    if "MODIFIERAS" in up or "IFRÅGASATT" in up or "UTMANAD" in up: return "amb", "◑", "MODIFIERAS — Analysen revideras"
...
"""
    ...
    if ranked:
        for hyp in ranked:
            ...
            styrka = (hyp.get("styrka") or "MEDEL").upper(); tes = hyp.get("tes", "")
            bevis = hyp.get("bevis", []) or []; motarg = hyp.get("motarg", []) or []; falsif = hyp.get("falsifiering", "")
            conf = hyp.get("conf", 0.5); pct = int(hyp.get("conf_pct", int(conf * 100))); color = STYRKA_COLOR.get(styrka, "#4a7fc0")
...
<div class="zone" style="margin-top:0;border-left:3px solid {color}">
  ...
  <div class="zone-body" style="padding:.8rem .9rem">
    ...
    <div style="margin-bottom:.7rem"><div class="hyp-sec-lbl">EVIDENS</div>{ev_html}</div>
...
""", unsafe_allow_html=True)
    with st.expander("Visa full råtext från primäranalysen", expanded=False):
        st.markdown(f'<div class="analysis-text">{_safe_links(_normalize_text_for_render(claude_answer))}</div>', unsafe_allow_html=True)

for key, val in [("result", None), ("running", False), ("awaiting_confirm", False), ("layers_generated", False), ("deep_generated", False)]:
                ...
                with c2:
                    try:
                        ...
                        if loaded2:
                            ...
                            st.download_button("PDF", pb, file_name=f"sanningsmaskinen_{e['timestamp'][:10]}_{slug}.pdf", mime="application/pdf", key=f"p_{e['filename']}")
                    except Exception:
                        pass
                with c3:
...
<div class="topbar">
  ...
  <div class="topbar-right">v8.17 · Claude Opus + GPT-4o · {today_str}</div>
</div>
...
question = st.text_area("", placeholder="Skriv en fråga — t.ex. Vem sprängde Nord Stream? eller Varför invaderade Ryssland Ukraina 2022?", height=84, label_visibility="collapsed")
...
if run_btn and question.strip():
    ...
    st.session_state.deep_generated = False
    try:
        from engine import event_reality_check, ask_claude, ask_gpt_critic, analyze_conflicts, run_red_team, auto_rewrite, assess_depth_recommendation
        steps = [("0 Reality", ""), ("1 Primär", ""), ("2 GPT", ""), ("3 Konflikt", ""), ("4 Red Team", ""), ("5 Rewrite?", "")]
        ph = st.empty()
        def upd(i, warn=-1):
            arr = []
            for j, (n, _) in enumerate(steps):
                if j < i: arr.append((n, "done"))
                elif j == i: arr.append((n, "active"))
                elif j == warn: arr.append((n, "warn"))
                else: arr.append((n, ""))
        ...
        result = {"question": question.strip(), "reality_check": rc, "claude_answer": ca, "gpt_answer": ga, "conflict_report": cf, "red_team_report": rr, "red_team_ok": red_team_ok, "collapsed": should_rewrite, "final_analysis": fa, "layers": {}, "degraded": not red_team_ok, "status": "DEGRADERAD" if not red_team_ok else ("REVIDERAD" if fa else "KLAR")}
    ...
    with c1:
        if st.button("Ja, fortsätt"):
            ...
            try:
                ...
                ca = ask_claude(q_tmp, rc_tmp); ga = ask_gpt_critic(q_tmp, ca, rc_tmp["status"]); cf = analyze_conflicts(ca, ga); rr, should_rewrite = run_red_team(q_tmp, ca, cf)
                red_team_ok = bool(rr and "misslyckades" not in rr.lower() and "api-fel" not in rr.lower() and len(rr) > 100)
                fa = auto_rewrite(q_tmp, ca, rr) if should_rewrite and red_team_ok else ""
                result = {"question": q_tmp, "reality_check": rc_tmp, "claude_answer": ca, "gpt_answer": ga, "conflict_report": cf, "red_team_report": rr, "red_team_ok": red_team_ok, "collapsed": should_rewrite, "final_analysis": fa, "layers": {}, "degraded": not red_team_ok, "status": "DEGRADERAD" if not red_team_ok else ("REVIDERAD" if fa else "KLAR")}
                result["depth_recommendation"] = assess_depth_recommendation(result)
                st.session_state.result = result
            except Exception as e:
                st.error(f"Fel: {e}")
...
if st.session_state.result:
    ...
    try:
        ...
        hyps = []
        st.markdown(f'<div class="degraded">NORMALIZER-KUNDE-INTE-PARSA HYPOTESER: {_safe(str(e))}</div>', unsafe_allow_html=True)
    ...
    rc_status = (rc.get("status", "") or "").upper(); res_status = (r.get("status", "") or "").upper()

    if r.get("degraded"):
        st.markdown('<div class="degraded">⚠ DEGRADERAD LEVERANS — Red Team körde inte korrekt. Lita inte på slutsatserna utan oberoende verifiering.</div>', unsafe_allow_html=True)

    rc_pill_cls, rc_pill_lbl = REALITY_PILL.get(rc_status, ("dim", rc_status)); st_pill_cls, st_pill_lbl = STATUS_PILL.get(res_status, ("dim", res_status))
    question_body = f"<div style=\"font-family:var(--serif);font-size:1.12rem;color:var(--ink);line-height:1.5;margin-bottom:.55rem;\">{_safe(r['question'])}</div><div style=\"display:flex;gap:.4rem;flex-wrap:wrap;\">{_pill(rc_pill_lbl, rc_pill_cls)}{_pill(st_pill_lbl, st_pill_cls)}</div>"
    ...
    if ranked:
        ...
        falsif = ranked[0].get("falsifiering", ""); falsif_html = f'<div class="exec-falsif"><span class="exec-falsif-lbl">VAD FALSIFIERAR?</span><span class="exec-falsif-txt">{_safe_links(falsif)}</span></div>' if falsif else ""
        exec_html = f"<div class=\"exec-conclusion\"><div class=\"exec-label\">SLUTSATS</div>{_safe(conclusion)}</div><div class=\"exec-explanation\">{_safe(explanation)}</div><div class=\"exec-conf\">AGGREGERAD KONFIDENS <span style=\"color:{conf_col};font-weight:700\">{avg_conf:.2f}</span> · {conf_lbl}</div>{falsif_html}"
        st.markdown(_section_zone("EXECUTIVE SUMMARY", exec_html, "grn", '<span style="font-size:.48rem;letter-spacing:.08em;color:var(--ink4)">ANALYTISK BEDÖMNING · INTE ETT SVAR</span>'), unsafe_allow_html=True)

    if ranked:
        hyp_html = _hyp_dashboard_html(ranked) + _nyckelord_html(ranked) + '<div class="metod-strip">Confidence = evidensstyrka × log(bevisantal+1) × källkvalitet, normaliserat 0–1. E5=officiell · E4=kvalitetsjournalistik · E3=rapport · E2=sekundär · E1=rykten</div>'
        st.markdown(_section_zone("HYPOTESER", hyp_html, "grn", '<span style="font-size:.48rem;color:var(--ink4)">EVIDENSSTYRKA · KLICKA FÖR DETALJER NEDAN</span>'), unsafe_allow_html=True)

    rc_text = rc.get("text", "") or rc.get("result", ""); rc_items = _parse_rc_structured(rc_text)
    ...
    st.markdown(_section_zone("REALITY CHECK", _rc_table_html(rc_items), rc_accent, _pill(rc_pill_lbl, rc_pill_cls)), unsafe_allow_html=True)

    if ranked:
        st.markdown('<div class="hyp-detail-label">Hypotes-detaljer — tes · evidens · motargument · falsifiering</div>', unsafe_allow_html=True)
        for hyp in ranked:
            key = hyp.get("key", ""); lbl = hyp.get("label", ""); title = hyp.get("title", "")
            styrka = (hyp.get("styrka") or "MEDEL").upper(); tes = hyp.get("tes", ""); bevis = hyp.get("bevis", []) or []; motarg = hyp.get("motarg", []) or []; falsif = hyp.get("falsifiering", "")
            conf = float(hyp.get("conf", 0.5)); pct = int(hyp.get("conf_pct", int(conf * 100))); color = STYRKA_COLOR.get(styrka, "#4a7fc0")
            ev_html = ('<ul class="hyp-ev-list">' + "".join(f"<li>{_safe_links(b)}</li>" for b in bevis[:5]) + "</ul>") if bevis else '<div class="hyp-sec-empty">Ingen evidens identifierad.</div>'
            mo_html = ("".join(f'<div class="hyp-mo">{_safe_links(m)}</div>' for m in motarg[:3])) if motarg else '<div class="hyp-sec-empty">Inga motargument identifierade.</div>'
            fl_html = f'<div class="hyp-fl">{_safe_links(falsif)}</div>' if falsif else '<div class="hyp-sec-empty">Inget falsifieringstest identifierat.</div>'
...
""", unsafe_allow_html=True)
    ...
    rr = r.get("red_team_report", ""); vc, vi, vt = _verdict_from_red_team(rr)
    if vc:
        st.markdown(f"""
...
""", unsafe_allow_html=True)
        ...
        with cl1:
            if st.button("📊 Layer 1–5"):
                with st.spinner("Bygger lagerstruktur..."):
                    ...
                    ground = deliver_ground_layers(r["question"], r["claude_answer"], r["gpt_answer"], r["red_team_report"], r["final_analysis"], rc)
    ...
    with st.expander("🔎 Reality Check — fullständig", expanded=False):
        st.markdown(f'<div class="analysis-text">{_safe_links(_normalize_text_for_render(rc.get("text","")))}</div>', unsafe_allow_html=True)
    with st.expander("⚔️ GPT-4 Kritiker", expanded=False):
        st.markdown(f'<div class="analysis-text">{_safe_links(_normalize_text_for_render(r.get("gpt_answer","")))}</div>', unsafe_allow_html=True)
    with st.expander("🎯 Red Team — fullständig rapport", expanded=False):
        st.markdown(f'<div class="analysis-text">{_safe_links(_normalize_text_for_render(r.get("red_team_report","")))}</div>', unsafe_allow_html=True)
    ...
    with c3:
        try:
            ...
            st.download_button("📄 PDF", _build_pdf(r), f"sanningsmaskinen_{today_str}_{slug}.pdf", "application/pdf", use_container_width=True)
        except Exception:
            pass
    ...
    st.markdown(f"""<div style="margin-top:1.5rem;padding-top:.5rem;border-top:1px solid var(--border);display:flex;justify-content:space-between;font-family:var(--mono);font-size:.55rem;color:var(--ink4);gap:1rem;flex-wrap:wrap;"><span>Sanningsmaskinen v8.17 · {today_str}</span><span>{_safe(rc_status)} · {_safe(res_status)}</span><span>Sanningen favoriserar ingen sida.</span></div>""", unsafe_allow_html=True)

elif not st.session_state.running and not st.session_state.awaiting_confirm:
    steps_info = [("Steg 0", "Reality Check — VERIFIED / ONGOING / HYPOTHETICAL / ANALYTICAL"), ("Steg 1", "Primäranalys — Claude Opus med tre hypoteser, bevis + falsifiering"), ("Steg 2", "GPT-4 destruktiv kritik — opponerar, sammanfattar inte"), ("Steg 3", "Konfliktanalys — epistemiska meningsskiljaktigheter"), ("Steg 4", "Red Team — VERDICT: HÅLLER / MODIFIERAS / KOLLAPSAR"), ("Steg 5", "Auto-rewrite om Red Team bedömer KOLLAPSAR eller MODIFIERAS"), ("Steg 6", "Layer 1–5 och fördjupningar — on-demand")]
