...
.step-active{
  ...
  background:#0b1220;
  box-shadow:
    0 0 0 1px rgba(110,182,255,.20) inset,
    0 0 10px rgba(110,182,255,.10),
    0 0 22px rgba(110,182,255,.08);
  animation:stepPulse 1.4s ease-in-out infinite !important;
}
.step-warn{
...
@keyframes stepPulse{
  0%{
    opacity:1;
    box-shadow:
      0 0 0 1px rgba(110,182,255,.18) inset,
      0 0 8px rgba(110,182,255,.05),
      0 0 14px rgba(110,182,255,.04);
  }
  50%{
    opacity:.68;
    box-shadow:
      0 0 0 1px rgba(110,182,255,.34) inset,
      0 0 16px rgba(110,182,255,.16),
      0 0 30px rgba(110,182,255,.12);
  }
  100%{
    opacity:1;
    box-shadow:
...
def _collect_links_from_hyp(hyp: dict):
    ...
    blocked_domains = {
        "google.com", "www.google.com", "google.se", "www.google.se",
    ...
    def _is_bad_url(u: str) -> bool:
        ...
        if not s.startswith("http"):
            return True
        if any(d in s for d in blocked_domains):
            return True
        if "/search?" in s or "q=" in s and "google." in s:
            return True
        if "localhost" in s:
            return True
        return False
    ...
    label = (hyp.get("label") or "").strip()
    key = (hyp.get("key") or "").strip()
    tes = (hyp.get("tes") or "").strip()
    ...
    if key and label:
        search_terms.append(f"{key} {label}")
    if tes:
        short_tes = " ".join(tes.split()[:12]).strip()
        if short_tes:
            search_terms.append(short_tes)
    ...
    for term in search_terms:
        if term and term not in seen_terms:
            seen_terms.add(term)
            fallback.append("https://www.google.com/search?q=" + quote_plus(term))
...
if run_btn and question.strip():
    ...
    try:
        ...
        red_team_ok = bool(
            rr
            and "misslyckades" not in rr.lower()
            and "api-fel" not in rr.lower()
            and len(rr) > 100
        )
        ...
        ph.markdown(
            step_html([(n, "done") for n, _ in steps]),
            unsafe_allow_html=True
        )
