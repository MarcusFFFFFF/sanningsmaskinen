...
.demo-hero{border:1px solid var(--border2);background:linear-gradient(180deg,var(--bg2) 0%,#10151d 100%);padding:1.15rem 1.25rem 1.05rem;margin:.35rem 0 1rem;position:relative;overflow:hidden}
.demo-hero::before{content:"";position:absolute;inset:0 auto 0 0;width:3px;background:var(--grn)}
.demo-kicker{font-family:var(--mono);font-size:.52rem;letter-spacing:.28em;text-transform:uppercase;color:var(--grn);margin-bottom:.45rem}
.demo-title{font-family:var(--serif);font-size:1.55rem;line-height:1.18;color:var(--ink);font-weight:700;max-width:14ch;margin-bottom:.55rem}
.demo-sub{font-family:var(--sans);font-size:.9rem;line-height:1.7;color:var(--ink2);max-width:62ch}
.demo-grid{display:grid;grid-template-columns:1.35fr .95fr;gap:1px;background:var(--border);margin:0 0 1rem}
.demo-card{background:var(--bg1);padding:1rem 1.1rem}
.demo-card h3{font-family:var(--mono);font-size:.54rem;letter-spacing:.24em;text-transform:uppercase;color:var(--ink3);margin:0 0 .65rem}
.demo-bullets{display:grid;gap:.52rem}
.demo-bullet{display:flex;gap:.55rem;align-items:flex-start;font-family:var(--sans);font-size:.86rem;line-height:1.55;color:var(--ink2)}
.demo-bullet b{color:var(--ink)}
.demo-dot{color:var(--grn);font-size:.8rem;line-height:1.4;flex-shrink:0}
.demo-note{border:1px solid var(--border);background:var(--bg1);padding:.85rem 1rem;margin:1rem 0 .9rem}
.demo-note strong{font-family:var(--mono);font-size:.54rem;letter-spacing:.2em;text-transform:uppercase;color:var(--blu);display:block;margin-bottom:.45rem}
.demo-note span{font-family:var(--sans);font-size:.86rem;line-height:1.6;color:var(--ink2)}
.question-shell{border:1px solid var(--border2);background:var(--bg1);padding:1rem 1rem .95rem;margin:.2rem 0 1rem}
.question-label{font-family:var(--mono);font-size:.52rem;letter-spacing:.24em;text-transform:uppercase;color:var(--ink3);margin-bottom:.6rem}
.example-row{display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.65rem}
.example-chip{font-family:var(--mono);font-size:.54rem;color:var(--ink3);border:1px solid var(--border);padding:.28rem .55rem;background:var(--bg2)}
@media (max-width:900px){.demo-grid{grid-template-columns:1fr}.demo-title{font-size:1.3rem;max-width:none}}
...
if not st.session_state.result and not st.session_state.running:
    st.markdown("""
    <div class="demo-hero">
      <div class="demo-kicker">Live-demo · epistemiskt analysverktyg</div>
      <div class="demo-title">Förstå komplexa frågor på minuter — inte timmar.</div>
      <div class="demo-sub">Sanningsmaskinen väger konkurrerande hypoteser, granskar evidens och visar vad som faktiskt har starkast stöd. Resultatet blir en tydlig lägesbild, inte bara mer information.</div>
    </div>
    <div class="demo-grid">
      <div class="demo-card">
        <h3>Vad du får direkt</h3>
        <div class="demo-bullets">
          <div class="demo-bullet"><span class="demo-dot">●</span><span><b>Bedömning:</b> vad som mest sannolikt händer just nu.</span></div>
          <div class="demo-bullet"><span class="demo-dot">●</span><span><b>Nyckelinsikt:</b> mekanismen bakom läget.</span></div>
          <div class="demo-bullet"><span class="demo-dot">●</span><span><b>Hypoteser:</b> H1, H2, H3 med evidensstyrka och falsifiering.</span></div>
        </div>
      </div>
      <div class="demo-card">
        <h3>Så testar du</h3>
        <div class="demo-bullets">
          <div class="demo-bullet"><span class="demo-dot">1</span><span>Skriv en komplex fråga med flera möjliga förklaringar.</span></div>
          <div class="demo-bullet"><span class="demo-dot">2</span><span>Klicka på <b>Analysera</b>.</span></div>
          <div class="demo-bullet"><span class="demo-dot">3</span><span>Läs först beslutslagret. Gå sedan djupare vid behov.</span></div>
        </div>
      </div>
    </div>
    <div class="demo-note">
      <strong>Passar bäst för</strong>
      <span>Geopolitik, samhällsfrågor, maktanalys, mediegranskning och andra frågor där flera konkurrerande förklaringar behöver vägas mot evidens.</span>
    </div>
    """, unsafe_allow_html=True)
    input_col, btn_col = st.columns([6, 1])
    with input_col:
        _placeholder = "Vem sprängde Nord Stream? · Varför invaderade USA Irak 2003? · Vad förklarar SDs framgångar?"
        st.markdown('<div class="question-shell"><div class="question-label">Ställ en fråga</div>', unsafe_allow_html=True)
        question = st.text_area("", placeholder=_placeholder,
                                height=115, label_visibility="collapsed",
                                key="main_question")
        st.markdown("""<div class="example-row">
            <span class="example-chip">Nord Stream</span>
            <span class="example-chip">Hormuz</span>
            <span class="example-chip">Irak 2003</span>
            <span class="example-chip">Svensk politik</span>
...
if st.session_state.result:
    ...
    # 2. EXECUTIVE SUMMARY — journalistisk, insight-first
    if ranked:
        ...
        # ── Bygg journalistisk insikttext från ranked + red team ──────────────
        w = ranked[0]
        w_key = w.get("key",""); w_lbl = w.get("label",""); w_title = w.get("title","")
        w_tes = w.get("tes","") or ""
        w_conf = float(w.get("conf",0.5))
        w_pct = int(w.get("conf_pct", int(w_conf*100)))

        # ── Ingress: situationsbeskrivning — INTE tes-texten ─────────────────
        # Genereras från reality check + ranked-kontext + frågan
        # Ska svara: vad ÄR situationen just nu, i en mening
        rc_status_now = (rc.get("status","") or "").upper()
        q_text = r.get("question","") or ""

        # Bygg ingress från reality check-text (första meningarna) + hypotes-läge
        rc_txt_raw = rc.get("text","") or ""
        ingress = ""

        # Sök en konkret faktamening ur reality check — inte en rubrik
        for line in rc_txt_raw.split('\n'):
            s = line.strip()
            s = re.sub(r'\*{1,3}', '', s).strip()
            s = re.sub(r'\[FAKTA\]|\[INFERENS\]|\[PÅGÅENDE\]|\[ONGOING\]', '', s, flags=re.IGNORECASE).strip()
            s = re.sub(r'ÖVERGRIPANDE STATUS.*', '', s, flags=re.IGNORECASE).strip()
            s = re.sub(r'^CLAIM\s*\d*:?\s*', '', s, flags=re.IGNORECASE).strip()
            s = re.sub(r'\|?\s*Status:.*', '', s).strip()
            s = re.sub(r'\[E[1-5][^\]]*\]', '', s).strip()
            s = re.sub(r'\[https?://[^\]]+\]', '', s).strip()
            s = re.sub(r'https?://\S+', '', s).strip()
            if len(s) > 60 and not s.startswith('#') and not s.startswith('**CLAIM') and not s.startswith('---'):
                ingress = s[:240].rstrip()
                if len(s) > 240:
                    cut = ingress.rfind('. ')
                    ingress = ingress[:cut+1] if cut > 80 else ingress + "…"
                break

        # Fallback: bygg ingress från frågan + status
        if not ingress:
            status_ord = {"ONGOING": "pågående", "VERIFIED": "bekräftad", "ANALYTICAL": "analytisk",
                          "HYPOTHETICAL": "hypotetisk", "PARTIAL": "delvis bekräftad", "UNVERIFIED": "obekräftad"}
            status_sv = status_ord.get(rc_status_now, "komplex")
            ingress = f"Situationen är {status_sv} och under förändring. {w_key} [{w_lbl}] har starkast evidensstöd ({w_pct}%)."

        # ── Nyckelinsikt: konkret analytisk wow-rad — ALDRIG samma som ingress ─
        # Sök i hela råtexten efter den mest citerbara meningen
        wow_line = ""
        wow_search_sources = []

        # 1. Sök i reviderad analys (starkast om den finns)
        final_txt = r.get("final_analysis","") or ""
        if final_txt.strip():
            wow_search_sources.append(final_txt)

        # 2. Sedan primäranalys
        raw_txt_full = r.get("claude_answer","") or ""
        wow_search_sources.append(raw_txt_full)

        WOW_KEYWORDS = [
            "saknar förmåga","kan inte öppna","asymmetrisk","hävstång","veto","filter",
            "stängt men","selektiv","räcker att hota","kontrollerar passagen",
            "inte militär","inte bara hot","upprätthålls med","degraderat","potentiellt kan hålla",
            "problem utan uppenbar lösning","ingen uppenbar lösning","real leverage",
            "1–6 månader","en till sex månader","95 procent","95%","räddade",
            "enda klara vinnaren","inga allierade","utan att kräva konventionell"
        ]

        for src in wow_search_sources:
            if wow_line: break
            for sent in re.split(r'(?<=[.!?])\s+', src):
                s = sent.strip()
                s = re.sub(r'\[FAKTA\]|\[INFERENS\]|\[PÅGÅENDE\]|\*{1,3}', '', s).strip()
                s = re.sub(r'\[[^\]]*\]\([^)]*\)', '', s).strip()
                if 50 < len(s) < 200 and any(kw.lower() in s.lower() for kw in WOW_KEYWORDS):
                    # Säkerställ att den INTE är identisk med ingress
                    if s[:60] not in ingress[:80]:
                        wow_line = s
                        break

        # Fallback nyckelinsikt från tes om inget annat hittats
        if not wow_line:
            tes_sents = [s.strip() for s in re.split(r'(?<=[.!?])\s+', w_tes) if 50 < len(s.strip()) < 200]
            for s in tes_sents:
                if s[:60] not in ingress[:80]:
                    wow_line = s
                    break
            if not wow_line and tes_sents:
                wow_line = tes_sents[0]

        # Red team alt-tes för kontext
        rr_text = r.get("red_team_report","") or ""
        alt_tes = ""
        for line in rr_text.split('\n'):
            s = line.strip()
            if re.match(r'\*{0,2}ALT-TES\*{0,2}', s, re.IGNORECASE):
                alt_tes = re.sub(r'\*{0,2}ALT-TES\*{0,2}:?\s*', '', s).strip()
                alt_tes = re.sub(r'^\*+|\*+$', '', alt_tes).strip()
                if len(alt_tes) > 160:
                    cut = alt_tes[:160].rfind(' ')
                    alt_tes = alt_tes[:cut] + "…"
                break

        # ── Konsekvenser — strukturerade per domän ───────────────────────────
        # Tre fasta domäner: ENERGI/EKONOMI · MILITÄRT/SÄKERHET · POLITISKT
        # Extraherar från hela råtexten (reviderad analys prioriterad)
        search_txt = (r.get("final_analysis","") or "") or (r.get("claude_answer","") or "")

        DOMAIN_CONFIG = [
            {
                "label": "ENERGI & EKONOMI",
                "color": "var(--amb)",
                "icon": "◈",
                "keywords": [
                    "pris","olja","brent","fat","energi","ekonomi","marknad","handel",
                    "export","import","lgn","gas","dollar","budg","produktion","försörjning",
                    "störning","inflation","recession","bnp","tillväxt"
                ]
            },
            {
                "label": "MILITÄRT & SÄKERHET",
                "color": "var(--red)",
                "icon": "◉",
                "keywords": [
                    "militär","anfall","attack","missil","drönar","fartyg","marin","flotta",
                    "centcom","pentagon","försvar","eskalat","krig","strid","bombning",
                    "minor","blockad","passage","stängt","öppna","konvoj","eskort"
                ]
            },
            {
                "label": "POLITISKT & DIPLOMATISKT",
                "color": "var(--blu)",
                "icon": "◇",
                "keywords": [
                    "trump","iran","usa","allierad","koalition","diplomati","förhandl",
                    "sanktion","ultimatum","president","nato","eu","kina","indien",
                    "politik","val","press","opinion","ledare","regering","utrikesminister"
                ]
            },
        ]

        def _extract_domain_consequence(text, keywords, max_len=150):
            """Hitta den mest konkreta meningen för en domän."""
            candidates = []
            for sent in re.split(r'(?<=[.!?])\s+', text):
                ...
                if len(s) < 50 or len(s) > 280: continue
                if s.startswith('|') or s.startswith('---'): continue
                kw_hits = sum(1 for kw in keywords if kw.lower() in s.lower())
                if kw_hits >= 2:
                    # Bonus för konkreta siffror/fakta
                    has_number = bool(re.search(r'\d', s))
                    candidates.append((kw_hits + (1 if has_number else 0), s))
            if not candidates:
                # En kw räcker som fallback
                for sent in re.split(r'(?<=[.!?])\s+', text):
                    s = sent.strip()
                    s = re.sub(r'\[FAKTA\]|\[INFERENS\]|\*{1,3}', '', s).strip()
                    s = re.sub(r'\[[^\]]*\]\([^)]*\)', '', s).strip()
                    s = re.sub(r'https?://\S+', '', s).strip()
                    if 50 < len(s) < 280 and any(kw.lower() in s.lower() for kw in keywords):
            ...
            if len(best) > max_len:
                cut = best[:max_len].rfind(' ')
                best = best[:cut] + "…" if cut > 60 else best[:max_len] + "…"
            return best

        # Kombinera breaking + fakta till en pool för domänextraction
        # Breaking-items prioriteras — de är de mest aktuella
        combined_pool = "\n".join(_breaking_pre + _fakta_pre)
        search_txt_enriched = combined_pool + "\n" + search_txt if combined_pool.strip() else search_txt

        domain_items = []
        for dom in DOMAIN_CONFIG:
            # Sök i enriched pool (breaking+fakta+full text)
            txt = _extract_domain_consequence(search_txt_enriched, dom["keywords"])
            if not txt:
                txt = _extract_domain_consequence(search_txt, dom["keywords"])
            if not txt:
                if dom["label"].startswith("ENERGI"):
                    txt = f"Global energimarknad under press — {w_key}-dynamiken driver prisutvecklingen"
                elif dom["label"].startswith("MILITÄRT"):
                    txt = f"Militärt läge olöst — {w_key} ({w_pct}%) pekar mot strukturell låsning"
                else:
                    txt = f"Politisk press eskalerar — se H2-dynamiken för inrikespolitiska drivkrafter"
            domain_items.append((dom["label"], dom["color"], dom["icon"], txt))
        ...
        # Analysera ranked[0] tes för vem som har övertaget
        upper_tes = w_tes.upper()
        if any(k in upper_tes for k in ["IRAN","TEHRAN","IRGC"]):
            if w_conf >= 0.60:
                overtag_kort = "Iran — asymmetrisk geografisk kontroll"
                overtag_medel = f"Oklart — beror på om {w_key}-dynamiken bryts"
            else:
                overtag_kort = "Oklart — evidensen är tunn"
                overtag_medel = "Beror på vilken hypotes som visar sig stämma"
        elif any(k in upper_tes for k in ["USA","TRUMP","PENTAGON"]):
            overtag_kort = "USA — militär kapacitet men politiska begränsningar"
            overtag_medel = "Beror på inrikespolitisk press och allierades agerande"
        else:
            overtag_kort = f"{w_key} — starkast stödd hypotes"
            overtag_medel = "Beror på hur det starkaste antagandet håller"

        # ── Bygg HTML ─────────────────────────────────────────────────────────

        # Domän-konsekvenser — tre rader med ikon + label + text
        domain_rows = ""
        for d_label, d_color, d_icon, d_txt in domain_items:
            domain_rows += (
                f'<div style="display:flex;gap:.6rem;align-items:flex-start;'
                f'padding:.38rem 0;border-bottom:1px solid var(--border)">'
                f'<span style="font-family:var(--mono);font-size:.55rem;color:{d_color};'
                f'flex-shrink:0;padding-top:.12rem">{d_icon}</span>'
                f'<div style="flex:1">'
                f'<div style="font-family:var(--mono);font-size:.42rem;letter-spacing:.2em;'
                f'color:{d_color};opacity:.8;margin-bottom:.1rem">{d_label}</div>'
                f'<div style="font-family:var(--sans);font-size:.79rem;color:var(--ink2);'
                f'line-height:1.6">{_safe(d_txt)}</div>'
                f'</div>'
                f'</div>'
            )
        cons_html = f'<div style="margin:.3rem 0">{domain_rows}</div>'
        wow_html = (
            f'<div style="margin:.8rem 0;padding:.65rem 1rem;'
            f'background:linear-gradient(90deg,#0a1711,#0d1d13);'
            f'border-left:4px solid var(--grn);border-top:1px solid var(--grn-dim);'
            f'border-bottom:1px solid var(--grn-dim)">'
            f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.35em;'
            f'color:var(--grn);margin-bottom:.3rem;text-transform:uppercase">⚡ Nyckelinsikt</div>'
            f'<div style="font-family:var(--serif);font-size:.95rem;font-weight:600;'
            f'color:#d4f0e0;line-height:1.55">{_safe(wow_line)}</div>'
            f'</div>'
        ) if wow_line else ""
        ...
        overtag_html = (
            ...
            f'background:var(--border);margin:.6rem 0">'
            f'<div style="background:var(--bg2);padding:.5rem .7rem">'
            f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.2em;'
            f'color:var(--ink4);margin-bottom:.2rem">KORT SIKT</div>'
            f'<div style="font-family:var(--sans);font-size:.8rem;color:var(--ink)">{_safe(overtag_kort)}</div>'
            f'</div>'
            f'<div style="background:var(--bg2);padding:.5rem .7rem">'
            f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.2em;'
            f'color:var(--ink4);margin-bottom:.2rem">MEDELLÅNG SIKT</div>'
            f'<div style="font-family:var(--sans);font-size:.8rem;color:var(--ink3)">{_safe(overtag_medel)}</div>'
            f'</div>'
            f'</div>'
        )

        # Alt-tes från red team
        alt_html = (
            f'<div style="padding:.45rem .8rem;background:var(--amb-bg);'
            f'border-left:2px solid var(--amb-dim);margin-top:.5rem">'
            f'<span style="font-family:var(--mono);font-size:.44rem;letter-spacing:.2em;'
            f'color:var(--amb)">RED TEAM ALT-TOLKNING  </span>'
            f'<span style="font-family:var(--sans);font-size:.78rem;color:#d4c080">{_safe(alt_tes)}</span>'
            f'</div>'
        ) if alt_tes else ""

        # Hypotes-minirad (under, som stöd)
        RANK_COLORS = ["var(--grn)","var(--amb)","var(--ink3)","var(--ink4)"]
        hyp_mini_rows = ""
        for i, h in enumerate(ranked[:3]):
            col = RANK_COLORS[i]
            k = h.get("key",""); lbl = h.get("label","")
            pct = int(h.get("conf_pct", int(h.get("conf",0.5)*100)))
            tes = h.get("tes","") or ""
            tes_short = tes[:110].rstrip()
            if len(tes) > 110:
                cut = tes_short.rfind(" ")
                tes_short = (tes_short[:cut] if cut > 55 else tes_short) + "…"
            hyp_mini_rows += (
                f'<div style="display:flex;gap:.6rem;align-items:baseline;padding:.28rem 0;'
                f'border-bottom:1px solid var(--border)">'
                f'<span style="font-family:var(--mono);font-size:.6rem;font-weight:700;'
                f'color:{col};min-width:3rem;flex-shrink:0">{_safe(k)}</span>'
                f'<span style="font-family:var(--mono);font-size:.5rem;color:var(--ink4);'
                f'min-width:2.2rem;flex-shrink:0">{pct}%</span>'
                f'<span style="font-family:var(--sans);font-size:.76rem;color:var(--ink3);'
                f'line-height:1.45">{_safe(tes_short)}</span>'
                f'</div>'
            )
        ...
        hyp_section_html = (
            ...
            f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.3em;'
            f'color:var(--ink4);text-transform:uppercase;margin-bottom:.35rem;'
            f'display:flex;align-items:center;gap:.5rem">'
            f'<span style="display:inline-block;width:10px;height:1px;background:var(--ink4)"></span>'
            f'Förklaringar — modellens stöd</div>'
            f'<div style="border-top:1px solid var(--border)">{hyp_mini_rows}</div>'
            f'</div>'
        )
        ...
        # Breaking-items i exec summary — samma data som primäranalysen
        exec_breaking_html = ""
        if _breaking_pre:
            b_rows = "".join(
                f'<li style="font-family:var(--sans);font-size:.8rem;color:#f2c0b0;'
                f'line-height:1.6;padding:.22rem 0;border-bottom:1px solid var(--red-dim);'
                f'list-style:none;display:flex;gap:.5rem;align-items:baseline">'
                f'<span style="color:var(--red);flex-shrink:0;font-size:.65rem">⚡</span>'
                f'<span>{_safe(b)}</span></li>'
                for b in _breaking_pre[:4]
            )
            exec_breaking_html = (
                f'<div style="background:var(--red-bg);border-left:3px solid var(--red);'
                f'padding:.45rem .8rem .3rem;margin-bottom:.3rem">'
                f'<div style="font-family:var(--mono);font-size:.42rem;letter-spacing:.3em;'
                f'color:var(--red);text-transform:uppercase;margin-bottom:.3rem">🔴 Breaking — senaste timmarna</div>'
                f'<ul style="margin:0;padding:0">{b_rows}</ul>'
                f'</div>'
            )

        exec_html = (
            # Breaking first — om det finns
            exec_breaking_html
            # Primär: journalistisk situationsingress
            + f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.3em;'
            f'color:var(--ink4);margin-bottom:.5rem;text-transform:uppercase">'
            f'ANALYS — EJ ETT FAKTAPÅSTÅENDE</div>'
            f'<div class="exec-conclusion"><div class="exec-label">DET SOM FAKTISKT HÄNDER NU</div>'
            f'{_safe(ingress)}</div>'
            f'{wow_html}'
            # Konsekvenser
            f'<div style="padding:.5rem 1rem;background:var(--bg2);border-top:1px solid var(--border)">'
            f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.28em;'
            f'color:var(--ink4);text-transform:uppercase;margin-bottom:.35rem">Vad detta betyder</div>'
            f'{cons_html}'
            f'</div>'
            # Övertag
            f'<div style="padding:.5rem 1rem;background:var(--bg1);border-top:1px solid var(--border)">'
            f'<div style="font-family:var(--mono);font-size:.44rem;letter-spacing:.28em;'
            f'color:var(--ink4);text-transform:uppercase;margin-bottom:.35rem">Vem har övertaget?</div>'
            f'{overtag_html}'
            f'</div>'
            # Alt-tolkning
            f'<div style="padding:0 1rem .3rem">{alt_html}</div>'
            # Hypoteser som stöd — längst ner
            f'<div style="padding:.2rem 1rem .6rem;background:var(--bg2);border-top:1px solid var(--border)">'
            f'{hyp_section_html}</div>'
            f'{winner_link_html}'
